...
//    }
    ...
    public void kodovanje(String tekst, String primalac) {
        try {
            ...
            byte[] kriptovanTekst = new byte[1 + potpisTeksta.length + tekstUByte.length];

            //tekst + potpis
            kriptovanTekst[0] = duzinaLozinke;
            System.arraycopy(potpisTeksta, 0, kriptovanTekst, 1, potpisTeksta.length);
            System.arraycopy(tekstUByte, 0, kriptovanTekst, potpisTeksta.length + 1, tekstUByte.length);
    ...
    public String dekodovanje(String putanjaDoslike) {
        ...
        try {
            ...
            //cita ostale bite
            if (duzinaPorukeInt % 3 == 1) {
                duzinaPorukeInt += 2;
            } else if (duzinaPorukeInt % 3 == 2) {
                duzinaPorukeInt++;
            }
            ...
            int duzinaPotpisa = niz[0];
            int pravaDuzinaPotpisa = 0;
            switch (duzinaPotpisa) {
                case 1:
                    pravaDuzinaPotpisa = 64;
                    break;
                case 2:
                    pravaDuzinaPotpisa = 128;
                    break;
                case 3:
                    pravaDuzinaPotpisa = 256;
                    break;
                case 4:
                    pravaDuzinaPotpisa = 512;
                    break;
                case 5:
                    pravaDuzinaPotpisa = 1024;
                    break;
                case 6:
                    pravaDuzinaPotpisa = 2048;
                    break;
            }

            byte[] potpisTeksta = new byte[pravaDuzinaPotpisa];
//                    Arrays.copyOfRange(niz, niz.length-256, niz.length);
//            byte [] potpisIzNiza = new byte[pravaDuzinaPotpisa];
            System.arraycopy(niz, 1, potpisTeksta, 0, pravaDuzinaPotpisa);

            byte[] tekst = new byte[niz.length - pravaDuzinaPotpisa - 1];
            System.arraycopy(niz, pravaDuzinaPotpisa + 1, tekst, 0, tekst.length);

            publicSignature.update(tekst);
            if (publicSignature.verify(potpisTeksta)) {
                rez = Helper.byteToString(tekst);
            } else {
                rez = "Poruka je neovlasteno promjenjena.";
            }
